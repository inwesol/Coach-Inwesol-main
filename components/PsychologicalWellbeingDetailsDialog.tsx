...
interface PsychologicalWellbeingData {
  id: string
  user_id: string
  session_id: number
  answers: string
...
interface PsychologicalWellbeingDetailsDialogProps {
  open: boolean
...
const PsychologicalWellbeingDetailsDialog: React.FC<
  PsychologicalWellbeingDetailsDialogProps
> = ({ open, onOpenChange, clientId, sessionId }) => {
  const [psychologicalWellbeingData, setPsychologicalWellbeingData] =
    useState<PsychologicalWellbeingData | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    if (open && clientId) {
      fetchPsychologicalWellbeingData()
    }
  }, [open, clientId])

  const fetchPsychologicalWellbeingData = async () => {
    try {
      setLoading(true)
      setError(null)

      const response = await fetch(
        `/api/session-details/${clientId}/psychological-wellbeing`
      )
      const data = await response.json()

      if (data.success) {
        setPsychologicalWellbeingData(data.data)
      } else {
        setError(data.error || 'Failed to fetch psychological wellbeing data')
      }
    } catch (err) {
      setError('Failed to fetch psychological wellbeing data')
      console.error('Error fetching psychological wellbeing data:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const getCompletionStatus = () => {
    if (!psychologicalWellbeingData) return 'Unknown'
    const answers = parseAnswers(psychologicalWellbeingData.answers)
      ...
      <DialogContent className='max-h-[90vh] max-w-5xl overflow-y-auto'>
        <DialogHeader>
          <DialogTitle className='flex items-center gap-2 text-2xl font-bold'>
            ...
            Psychological Wellbeing Assessment Details
          </DialogTitle>
        </DialogHeader>
        ...
        {loading && (
          <div className='flex items-center justify-center py-8'>
            ...
            <span className='ml-2'>
              Loading psychological wellbeing data...
            </span>
          </div>
        )}
        ...
        {error && (
          <div className='py-8 text-center'>
            ...
            <p className='text-muted-foreground'>
              No psychological wellbeing data available for this client.
            </p>
          </div>
        )}

        {psychologicalWellbeingData && !loading && (
          <div className='space-y-6'>
              ...
              <CardContent className='space-y-4'>
                <div className='grid grid-cols-1 gap-4 md:grid-cols-3'>
                  <div>
                    ...
                    <p className='text-lg'>
                      {psychologicalWellbeingData.session_id}
                    </p>
                  </div>
                  <div>
                    <label className='text-sm font-medium text-muted-foreground'>
                  ...
                  <div>
                    ...
                    <div className='mt-1'>
                      <span
                        className={`text-2xl font-bold ${getScoreLevel(parseFloat(psychologicalWellbeingData.score)).color}`}
                      >
                        {psychologicalWellbeingData.score}
                      </span>
            ...
            {/* Subscale Scores */}
            {psychologicalWellbeingData.subscale_scores &&
              Object.keys(psychologicalWellbeingData.subscale_scores).length >
                0 && (
                <Card>
                  <CardHeader>
                    <CardTitle className='flex items-center gap-2'>
              ...
              <CardContent>
                {(() => {
                  const answers = parseAnswers(
                    psychologicalWellbeingData.answers
                  )
                  if (!answers) {
                    return (
                      <div className='py-4 text-center'>
                        <p className='text-muted-foreground'>
                          No psychological wellbeing responses available or data
...
export default PsychologicalWellbeingDetailsDialog
