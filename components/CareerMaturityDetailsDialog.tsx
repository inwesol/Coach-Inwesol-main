...
interface CareerMaturityDetailsDialogProps {
  open: boolean
...
> = ({ open, onOpenChange, clientId, sessionId }) => {
  const [careerMaturityData, setCareerMaturityData] =
    useState<CareerMaturityData | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    if (open && clientId) {
      fetchCareerMaturityData()
    }
  }, [open, clientId])

  const fetchCareerMaturityData = async () => {
    try {
      setLoading(true)
      setError(null)

      const response = await fetch(
        `/api/session-details/${clientId}/career-maturity`
      )
      const data = await response.json()

      if (data.success) {
        setCareerMaturityData(data.data)
      } else {
        setError(data.error || 'Failed to fetch career maturity data')
      }
    } catch (err) {
      setError('Failed to fetch career maturity data')
      console.error('Error fetching career maturity data:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const getCompletionStatus = () => {
    if (!careerMaturityData) return 'Unknown'
    const answers = parseAnswers(careerMaturityData.answers)
      ...
      <DialogContent className='max-h-[90vh] max-w-5xl overflow-y-auto'>
        <DialogHeader>
          <DialogTitle className='flex items-center gap-2 text-2xl font-bold'>
            ...
            Career Maturity Assessment Details
          </DialogTitle>
        </DialogHeader>
        ...
        {loading && (
          <div className='flex items-center justify-center py-8'>
            ...
            <span className='ml-2'>Loading career maturity data...</span>
          </div>
        )}
        ...
        {error && (
          <div className='py-8 text-center'>
            ...
            <p className='text-muted-foreground'>
              No career maturity data available for this client.
            </p>
          </div>
        )}
              ...
              <CardContent>
                <div className='flex items-center justify-between gap-4'>
                  <div className='flex items-center gap-2'>
                    ...
                    <p className='text-sm font-medium'>{careerMaturityData.session_id}</p>
                  </div>
                  <div className='flex items-center gap-2'>
                    <label className='text-sm font-medium text-muted-foreground'>
                      Status:
                    ...
                    careerMaturityData.score !== null && (
                      <div className='flex items-center gap-2'>
                        ...
                        <span className='text-xl font-bold text-blue-600'>
                          {Math.round(careerMaturityData.score)}%
                        </span>
                      </div>
                    )}
                </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className='flex items-center gap-2'>
                  ...
                  Career Maturity Assessment Responses
                </CardTitle>
              </CardHeader>
              <CardContent>
                {(() => {
                  ...
                  if (!answers) {
                    return (
                      <div className='py-4 text-center'>
                        <p className='text-muted-foreground'>
                          No career maturity responses available or data is not
                          in expected format.
                        </p>
                      </div>
                    )
...
export default CareerMaturityDetailsDialog
